  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4">
        ...
        <span className="text-stone text-xs font-semibold">Loading visual builder...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 h-full flex flex-col overflow-hidden relative">
      ...
      <div className="flex justify-between items-center flex-shrink-0">
        <div className="flex items-center gap-3">
          <button 
            ...
            className="p-2 bg-canvas border border-line rounded-xl hover:bg-surface text-stone hover:text-cream transition-colors"
          >
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <h2 className="text-xl font-bold flex items-center gap-2">
              <Zap className="w-5 h-5 text-gold" /> Automation Designer
            </h2>
            <p className="text-xs text-stone mt-0.5">Design multi-channel flow trigger diagrams.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button 
            ...
            disabled={saving}
            className="px-4 py-2 text-xs font-bold rounded-xl bg-gold hover:bg-gold-light disabled:bg-surface text-white flex items-center gap-1.5 shadow-lg transition-colors"
          >
      ...
      <div className="flex-1 flex gap-6 overflow-hidden min-h-[500px]">
        ...
        <div className="flex-1 glass border border-line rounded-lg p-6 overflow-y-auto flex flex-col gap-6 bg-surface">
          
          {/* Workflow Name Editor card */}
          <div className="p-4 bg-canvas/30 rounded-lg border border-line space-y-3 flex-shrink-0">
            <label className="text-[10px] font-bold text-stone-400 uppercase tracking-wide block">Workflow Campaign Name</label>
            <div className="relative">
              <Edit3 className="absolute left-3 top-3 w-4 h-4 text-stone-400" />
              <input 
                type="text" 
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="e.g. Instant Call Qualifier for Facebook Leads" 
                className="w-full bg-ink border border-line rounded-xl pl-10 pr-4 py-2.5 text-xs text-cream font-bold focus:border-gold focus:outline-none"
              />
            </div>
          </div>

          {/* Trigger Card Node */}
          <div className="relative pl-6 before:absolute before:left-2.5 before:top-6 before:bottom-0 before:w-0.5 before:bg-canvas flex-shrink-0">
            <span className="absolute left-0 top-1 w-5 h-5 rounded-full bg-gold flex items-center justify-center font-bold text-[9px] text-white">⚡</span>
            <div className="text-[10px] font-bold text-gold font-medium mb-2">Campaign Entry Node</div>
            <div className="glass p-4 rounded-xl border border-line max-w-lg">
              <h4 className="font-bold text-xs text-white">Trigger Source</h4>
              <div className="flex items-center gap-3 mt-3">
                <select 
                  value={trigger.type} 
                  onChange={(e) => setTrigger({ ...trigger, type: e.target.value })}
                  className="bg-canvas border border-line rounded-xl px-3 py-2 text-xs text-stone-400 focus:outline-none w-56"
                >
                ...
                <span className="text-[11px] text-stone-400">Activates automation instantly.</span>
              </div>
            </div>
          </div>

          {/* Intermediate Steps timeline */}
          <div className="space-y-6 pl-6 relative before:absolute before:left-2.5 before:top-0 before:bottom-0 before:w-0.5 before:bg-canvas">
            {steps.map((step, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`group relative glass rounded-xl border transition-all cursor-pointer p-4 max-w-lg flex items-center gap-4 hover:border-line/80 ${
                    isSelected ? 'border-gold/70 bg-gold-muted ring-1 ring-gold/30' : 'border-line'
                  }`}
                >
                  {/* Step index counter indicator */}
                  <span className={`absolute -left-[23px] top-4 w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-semibold text-white transition-colors ${
                    isSelected ? 'bg-gold' : 'bg-surface border border-line'
                  }`}>
                    {idx + 1}
                  </span>

                  {/* Drag Grip Icon */}
                  <div className="cursor-grab text-stone-600 group-hover:text-stone-400 active:cursor-grabbing">
                  ...
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    ...
                    step.type === 'send_email' ? 'bg-gold-muted text-gold border border-gold/20' :
                    step.type === 'wait' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' :
                    step.type === 'assign_lead' ? 'bg-sage-muted text-sage-light border border-teal-500/20' :
                    step.type === 'change_stage' ? 'bg-sage-muted text-sage-light border border-sage/20' :
                    'bg-surface text-stone border border-line'
                  }`}>
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="font-bold text-xs text-cream flex items-center gap-1.5 capitalize">
                  ...
                  <button 
                    ...
                    className="p-1.5 opacity-0 group-hover:opacity-100 hover:bg-canvas border border-transparent hover:border-line rounded-lg text-stone-400 hover:text-red-400 transition-all flex-shrink-0"
                    title="Delete step"
                  >
          ...
          <div className="pl-6 pt-2 flex-shrink-0">
            <div className="relative max-w-lg">
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-stone-400 uppercase tracking-wide">Add Flow Step:</span>
                <div className="flex flex-wrap gap-1.5">
                  <button onClick={() => addStepNode('make_ai_call')} className="px-2.5 py-1.5 rounded-xl border border-line bg-surface text-[10px] text-stone hover:text-emerald-400 hover:border-emerald-500/30 transition-colors font-semibold flex items-center gap-1">
                    <Bot className="w-3 h-3" /> Call
                  </button>
                  <button onClick={() => addStepNode('send_whatsapp')} className="px-2.5 py-1.5 rounded-xl border border-line bg-surface text-[10px] text-stone hover:text-green-400 hover:border-green-500/30 transition-colors font-semibold flex items-center gap-1">
                    <MessageSquare className="w-3 h-3" /> WhatsApp
                  </button>
                  <button onClick={() => addStepNode('send_email')} className="px-2.5 py-1.5 rounded-xl border border-line bg-surface text-[10px] text-stone hover:text-gold hover:border-gold/30 transition-colors font-semibold flex items-center gap-1">
                    <Mail className="w-3 h-3" /> Email
                  </button>
                  <button onClick={() => addStepNode('wait')} className="px-2.5 py-1.5 rounded-xl border border-line bg-surface text-[10px] text-stone hover:text-amber-400 hover:border-amber-500/30 transition-colors font-semibold flex items-center gap-1">
                    <Clock className="w-3 h-3" /> Wait
                  </button>
                  <button onClick={() => addStepNode('assign_lead')} className="px-2.5 py-1.5 rounded-xl border border-line bg-surface text-[10px] text-stone hover:text-sage-light hover:border-teal-500/30 transition-colors font-semibold flex items-center gap-1">
                    <UserCheck className="w-3.5 h-3.5" /> Assign
                  </button>
                  <button onClick={() => addStepNode('change_stage')} className="px-2.5 py-1.5 rounded-xl border border-line bg-surface text-[10px] text-stone hover:text-sage-light hover:border-sage/30 transition-colors font-semibold flex items-center gap-1">
                    <Layers className="w-3.5 h-3.5" /> Stage
                  </button>
                  <button onClick={() => addStepNode('webhook')} className="px-2.5 py-1.5 rounded-xl border border-line bg-surface text-[10px] text-stone hover:text-sky-400 hover:border-sky-500/30 transition-colors font-semibold flex items-center gap-1">
        ...
        <div className="w-80 glass border border-line rounded-lg p-6 overflow-y-auto bg-surface">
          <div className="flex justify-between items-center border-b border-line pb-3 flex-shrink-0">
            <h3 className="font-bold text-xs text-white font-medium flex items-center gap-1.5">
              <Settings className="w-4 h-4 text-gold" /> Card properties
            </h3>
            {selectedStepIndex !== null && (
              <span className="text-[10px] text-stone-400 font-bold">Step {selectedStepIndex + 1}</span>
            )}
          </div>
            ...
            {selectedStepIndex !== null && steps[selectedStepIndex] ? (
              <div className="space-y-5 animate-in fade-in duration-200">
                ...
                {steps[selectedStepIndex].type === 'wait' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Delay Duration</label>
                      <input 
                        type="number" 
                        value={steps[selectedStepIndex].delay || ''}
                        onChange={(e) => updateStepProperty(selectedStepIndex, 'delay', parseInt(e.target.value) || 0)}
                        className="w-full bg-canvas border border-line rounded-xl px-3 py-2 text-xs text-white focus:outline-none"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Time Unit</label>
                      <select 
                ...
                {steps[selectedStepIndex].type === 'make_ai_call' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Wait Delay before dialing (s)</label>
                      <input 
                        type="number" 
                ...
                {steps[selectedStepIndex].type === 'send_whatsapp' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Meta Notification Template</label>
                      <select 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Variable (e.g. Lead Name)</label>
                      <input 
                        type="text" 
                ...
                {steps[selectedStepIndex].type === 'send_email' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Transaction Template ID</label>
                      <select 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Email Subject line</label>
                      <input 
                        type="text" 
                ...
                {steps[selectedStepIndex].type === 'assign_lead' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Assign Criteria</label>
                      <select 
                ...
                {steps[selectedStepIndex].type === 'change_stage' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">Shift Target CRM Stage</label>
                      <select 
                ...
                {steps[selectedStepIndex].type === 'webhook' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-stone-400 uppercase">POST URL</label>
                      <input 
                        type="url" 
              ...
              <div className="text-center py-20 text-[11px] text-stone-600">
